...
def construir_notificaciones_reinversion(df_inv: pd.DataFrame, df_config: pd.DataFrame,
                                          fecha_hoy: "pd.Timestamp | None" = None,
                                          horizonte_dias: int = 90) -> list:
    """
    Feed de avisos "hacia delante" para el Dashboard: no solo lo que toca reinvertir HOY (eso ya
    lo cubre calcular_reinversiones_pendientes), sino una PROYECCIÓN de cuándo tocará cada
    reinversión y cuánto capital habrá que reinvertir, dentro del horizonte indicado (90 días
    por defecto). Pensado para mostrarse como noticias del fondo: "el día X tocará reinvertir a
    Fulanito ~$Y".

    - PERIODICO: la fecha es exacta (fecha_ultima_reinversion + N meses); el importe se estima
      con el mismo cálculo de intereses que usan los extractos, proyectado a esa fecha futura
      asumiendo que el capital activo del inversor no cambia mientras tanto.
    - UMBRAL: no hay fecha fija, así que se estima a partir del ritmo de generación de interés
      actual (capital activo × tasa media ÷ 365 = interés diario) cuánto falta para alcanzar el
      importe configurado, y se proyecta esa fecha. Es una ESTIMACIÓN: si cambia el capital
      activo del inversor, la fecha real se adelanta o se atrasa.
    """
    import math
    hoy = pd.Timestamp(fecha_hoy).normalize() if fecha_hoy is not None else pd.Timestamp.today().normalize()
    notificaciones = []
    ...
    for _, cfg in df_config.iterrows():
        ...
        fecha_ultima = cfg["fecha_ultima_reinversion"]
        if pd.isna(fecha_ultima):
            info_ini = calcular_intereses_acumulados_inversor(df_inv, inversor, fecha_fin=hoy)
            fecha_ultima = info_ini.get("fecha_inicio") or hoy

        info_hoy = calcular_intereses_acumulados_inversor(df_inv, inversor, fecha_fin=hoy)
        acumulado_hasta_hoy = info_hoy["total_intereses"]
        acumulado_hasta_ultima = calcular_intereses_acumulados_inversor(df_inv, inversor, fecha_fin=fecha_ultima)["total_intereses"]
        intereses_periodo = round(acumulado_hasta_hoy - acumulado_hasta_ultima, 2)

        fecha_estimada = pd.NaT
        importe_estimado = 0.0

        if tipo == "PERIODICO":
            fecha_estimada = _proximo_disparo_periodico(fecha_ultima, valor)
            if pd.notna(fecha_estimada):
                importe_estimado = round(
                    calcular_intereses_acumulados_inversor(df_inv, inversor, fecha_fin=fecha_estimada)["total_intereses"]
                    - acumulado_hasta_ultima, 2
                )
        elif tipo == "UMBRAL" and valor > 0:
            if intereses_periodo >= valor:
                fecha_estimada = hoy
                importe_estimado = intereses_periodo
            else:
                capital_activo = info_hoy["capital_activo"]
                tasa_media = info_hoy["tasa_media"]
                interes_diario = capital_activo * tasa_media / 365
                if interes_diario > 0:
                    dias_para = math.ceil((valor - intereses_periodo) / interes_diario)
                    fecha_estimada = hoy + pd.Timedelta(days=int(dias_para))
                    importe_estimado = valor

        if pd.isna(fecha_estimada):
            continue
        dias_para = (fecha_estimada.normalize() - hoy).days
        if dias_para > horizonte_dias:
            continue
        notificaciones.append({
            "inversor": inversor,
            "tipo_disparador": tipo,
            "fecha_estimada": fecha_estimada.normalize(),
            "dias_para": dias_para,
            "importe_estimado": round(importe_estimado, 2),
        })

    notificaciones.sort(key=lambda n: n["dias_para"])
    return notificaciones


def mostrar_feed_notificaciones_reinversion():
    """Pestaña 'Notificaciones' del Dashboard: feed tipo noticias con las reinversiones que
    tocan ya o que se estima que tocarán en los próximos 90 días, calculado en cada carga —
    no requiere ninguna acción manual."""
    df_inv, _, _ = cargar_excel_completo()
    df_config = leer_config_reinversion()

    if df_config.empty:
        st.info("Todavía no hay ningún inversor con configuración de reinversión. Configúralo en el menú '🔁 Reinversiones' → 'Configuración por inversor'.")
        return

    notificaciones = construir_notificaciones_reinversion(df_inv, df_config, horizonte_dias=90)
    if not notificaciones:
        st.success("No hay ninguna reinversión prevista en los próximos 90 días.")
        return

    for n in notificaciones:
        fecha_str = n["fecha_estimada"].strftime("%d/%m/%Y")
        importe_str = f"${n['importe_estimado']:,.2f}"
        etiqueta_tipo = "periódica" if n["tipo_disparador"] == "PERIODICO" else "por umbral"
        if n["dias_para"] <= 0:
            st.error(f"🔴 **Hoy** — {n['inversor']}: toca reinvertir (reinversión {etiqueta_tipo}) — importe estimado **{importe_str}**.")
        elif n["dias_para"] <= 7:
            st.warning(f"🟠 **{fecha_str}** (en {n['dias_para']} días) — {n['inversor']}: reinversión {etiqueta_tipo} — importe estimado **{importe_str}**.")
        elif n["dias_para"] <= 30:
            st.info(f"🔵 **{fecha_str}** (en {n['dias_para']} días) — {n['inversor']}: reinversión {etiqueta_tipo} — importe estimado **{importe_str}**.")
        else:
            st.markdown(f"⚪ **{fecha_str}** (en {n['dias_para']} días) — {n['inversor']}: reinversión {etiqueta_tipo} — importe estimado {importe_str}.")
...
def dashboard_financiero():
        ...
        try:
            ...
            if not _df_cfg_reinv.empty:
                ...
                if not _disparadas_reinv.empty:
                    st.warning(f"🔁 {len(_disparadas_reinv)} inversor(es) tienen una reinversión pendiente hoy — mira la pestaña '🔔 Notificaciones' más abajo.")
        except Exception:
            pass
    ...
    with tab6:
        st.caption("Avisos de reinversión de intereses, calculados en cada carga del dashboard: lo que toca hoy y lo previsto en los próximos 90 días.")
        mostrar_feed_notificaciones_reinversion()
    with tab4:
