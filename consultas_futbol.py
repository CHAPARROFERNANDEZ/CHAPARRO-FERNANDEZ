...
HOJA = "INVERSIONES"
TASA_ANUAL_FUTBOL = 0.15   # 15% anual
...
def filtrar_futbol(df_base):
    return df_base[
        df_base["subtipo_inversion"].astype(str).str.lower().eq("futbol")
        | df_base["subtipo_inversion"].astype(str).str.lower().eq("fútbol")
        | df_base["nombre_activo"].astype(str).str.lower().eq("futbol")
        | df_base["nombre_activo"].astype(str).str.lower().eq("fútbol")
    ].copy()
...
def mostrar_menu():
    print("\n" + "=" * 72)
    print("CONSULTAS FÚTBOL / INVERSORES")
    print("=" * 72)
    print("1. ¿Cuánto ingresará Fútbol en un mes?")
    ...
    print("6. ¿Cuánto capital hay actualmente activo en Fútbol hoy?")
    print("7. ¿Cuánto capital había activo en Fútbol en un mes concreto?")
    print("8. Salir")
    print("=" * 72)
...
def main():
    while True:
        ...
        if opcion == "1":
            ...
            total, detalle = ingresos_futbol_mes(df, anio, mes)
            print(f"\nIngresos de Fútbol en {nombre_mes_es(mes)} {anio}: {fmt(total)}")
            ...
            if not detalle.empty:
                ...
                if ver == "s":
                    mostrar = detalle[[
                        ...
                        "capital_invertido",
                        "dias_activos",
                        "ingreso_bruto_futbol"
                    ]].copy()
                    mostrar["capital_invertido"] = mostrar["capital_invertido"].map(fmt)
                    mostrar["ingreso_bruto_futbol"] = mostrar["ingreso_bruto_futbol"].map(fmt)
            ...
            bruto = capital_activo_futbol_hoy(df, solo_real=False)
            real = capital_activo_futbol_hoy(df, solo_real=True)

            print(f"\nCapital actualmente activo en Fútbol: {fmt(bruto)}")
            print(f"Capital actualmente activo en Fútbol (solo capital_nuevo_real = si): {fmt(real)}")

        elif opcion == "7":
            anio, mes = pedir_anio_mes()
            bruto = capital_activo_futbol_en_mes(df, anio, mes, solo_real=False)
            real = capital_activo_futbol_en_mes(df, anio, mes, solo_real=True)

            print(f"\nCapital activo en Fútbol al cierre de {nombre_mes_es(mes)} {anio}: {fmt(bruto)}")
            print(f"Capital activo en Fútbol al cierre de {nombre_mes_es(mes)} {anio} (solo capital_nuevo_real = si): {fmt(real)}")
